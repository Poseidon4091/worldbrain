...
reason: "Forward-looking plan to expose the world as an MCP context layer (a self-made Unabyss). Not implemented yet."
---

# World as a Self-Made "Unabyss" — MCP Context Layer (Plan)

## 1. Vision

The world system is already a self-updating, structured, relevance-ranked **narrative memory**:
...
- **Scope per-user / per-world.** Reuse the ownership checks already in `worldService`.
...
- **Calls the same functions the pipeline uses** (this is the whole point):
  - Retrieval: `hybridSearch`, `dciLookupByNames` / `dciAugmentRetrievedItems`,
    `tagGatedSearch` (with the new `tagAppearsInText`).
  - Structure: `worldService` (`list` / `get` / `getIndexedItems` / `getCheckpoints`).
  - Query embedding: `embedText` with the user's embedding router/model from Settings (same as
    `buildPrompt`).
- **Auth:** a per-user MCP token → `userId`. Every tool call is scoped to that user's worlds,
  mirroring `worldService`'s existing ownership guard. Reuse the BYOK `apiKey` infra or a small
...
| Tool | Maps to | Notes |
|---|---|---|
| `worlds.list()` | `worldService.list` | id, title, tags, group, updatedAt, entity counts |
| `world.entity(worldId, name)` | `dciLookupByNames` | exact-name card; resolves **aliases** so nicknames work |
| `world.search(worldId?, query, limit)` | `embedText` → `hybridSearch` + `dciAugmentRetrievedItems` | ranked hits tagged `source: vector\|tag\|dci` |
...
- **Canon gating:** optional `canonPosition` param (book/chapter) → `hybridSearch`, reusing
...
- **M2 — Connections *in* (client direction).** Generalize `GraphLorebookRef` so a "reference" can be
  ...
  connect" half.
- **M3 — Write over MCP (careful).** `world.propose(worldId, delta)` → runs through `applyDelta`
...
> The world is already the structured, self-updating, relevance-ranked store Unabyss charges for.
