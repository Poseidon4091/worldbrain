...
It's the lorebook framework extracted from Aria, un-coupled from the chat app and repurposed as a
...
| **DCI** | `services/lorebook/lorebookDciSearch.ts` | exact-name recall, alias/nickname + word-boundary matching, mention detection |
| **Merge** | `services/lorebook/lorebookMerge.ts` | delta application, fuzzy dedup, death/tombstone/fixedContent protection |
| **Lifecycle** | `services/lorebook/lorebookLifecycle.ts` | recency stamping, chronology rollup, entity dormancy (bounded working memory) |
| **Sync** | `services/lorebook/lorebookItemSync.ts`, `rpPassageSync.ts` | checkpoint → relational items + embeddings; passage chunking |
...
| **Worlds** | `services/lorebook/worldService.ts` | standalone world CRUD (no chat/graph coupling) |
...
Ported from the Aria lorebook subsystem. The retrieval/merge/lifecycle/ingest logic is the same,
